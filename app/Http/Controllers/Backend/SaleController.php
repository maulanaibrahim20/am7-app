...
class SaleController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->filled('status')) {
            $rows->where('payment_status', $request->status);
        }
        ...
        return DataTables::of($rows)
            ...
            ->filterColumn('cashier_name', function ($query, $keyword) {
                $query->whereHas('cashier', function ($q) use ($keyword) {
                    $q->where('name', 'like', "%{$keyword}%");
                });
            })
            ->editColumn('invoice_number', function ($row) {
                $url = route('sale.show', $row->id);
            ...
            ->editColumn('payment_method', fn($r) => ucfirst($r->payment_method))
            ...
            ->addColumn('action', function ($r) {
                if ($r->payment_status !== 'paid') {
                    $url = url('sale.edit', $r->id);
                    return '<a href="' . $url . '" class="btn btn-sm btn-info" data-toggle="ajaxModal"
                    data-title="Update Payment ' . e($r->invoice_number) . '">
                    <i class="fas fa-cash-register me-1"></i> Pay</a>';
                }
    ...
    {
        return view('app.backend.pages.sale.filter');
    }
    ...
    public function show($id)
    {
        ...
        $stockMovements = StockMovement::where('reference_type', Sale::class)
            ->where('reference_id', $sale->id)
            ->with('product')
            ->get();

        return view('app.backend.pages.sale.show', compact('sale', 'stockMovements'));
    }
}
